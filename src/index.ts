...
app.get("/openapi.yaml", (c) => {
	...
	if (!specPath) return c.json(notFound("file", "openapi.yaml"), 404)
...
api.get("/categories/:slug", (c) => {
	...
	if (!category) return c.json(notFound("category", slug), 404)
	const items = (getCategoryItems(store, slug) ?? []).map((it) =>
		enrichItemAudio(it, reciter),
	)
...
api.get("/categories/:slug/items", (c) => {
	...
	if (!items) return c.json(notFound("category", slug), 404)
	return c.json({
		slug,
		total: items.length,
...
api.get("/categories/:slug/items/:index", (c) => {
	...
	if (Number.isNaN(index)) return c.json(badRequest("index must be a number"), 400)
	const item = getItem(store, slug, index)
	if (!item) return c.json(notFound("item", `${slug}#${index}`), 404)
...
api.get("/audio/ayah/:surah/:ayah", (c) => {
	...
	if (!resolved) return c.json(notFound("reciter", reciter ?? "<any>"), 404)
...
api.get("/search", (c) => {
	...
	if (!q) return c.json(badRequest("query parameter 'q' is required"), 400)
	return c.json(
		searchItems(store, q, {
...
api.get("/random", (c) => {
	...
	if (!item) return c.json(notFound("category", category ?? "<any>"), 404)
	return c.json(item)
})
